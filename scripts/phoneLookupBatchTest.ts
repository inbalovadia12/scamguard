...
function scoreCase(expected: Expected, actual: any) {
  ...
  const numberEcho = normalize(actual.phone_number || actual.normalized_number);
  if (numberEcho && !numberEcho.endsWith(normalize(expected.number))) {
...
Deno.serve(async (req) => {
  try {
    ...
    const cases: Expected[] = Array.isArray(body.cases) && body.cases.length ? body.cases.slice(0, 25) : DEFAULT_CASES;

    const results = [];
    for (const testCase of cases) {
      const started = Date.now();
      try {
        // This deliberately calls the same production function rather than duplicating
        // its research logic. It therefore tests the real endpoint contract, prompts,
        // evidence merging, scoring, country detection, and database reconciliation.
        ...
        results.push({
          ...
          ok: scoreCase(testCase, result).length === 0,
          failures: scoreCase(testCase, result),
    ...
    return Response.json({
      ...
      results,
      note: 'This is an admin-only live regression harness. It intentionally uses the production lookup function and therefore consumes normal lookup credits.',
    });
  } catch (error) {
