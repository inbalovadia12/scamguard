...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const prompt = mode === 'address'
      ...
      : `You are Vardin's AI message-checking engine. Analyze the COMPLETE crypto DM/investment/giveaway message below, using the same whole-message reasoning standard as Vardin Message Check. Never reduce the verdict to the token name or to one isolated keyword.\n\nMESSAGE:\n"""${input}"""\n\n${urlContext ? `LIVE URL SCAN FOR THE LINK IN THIS MESSAGE:\n${JSON.stringify(urlContext)}\n\nUse this URL evidence as additional context, but still judge the COMPLETE MESSAGE. A clean, unknown, or failed URL scan does NOT make a suspicious message safe, and a suspicious URL should strengthen the risk assessment when the message asks the user to visit it.\n` : 'If the message contains a URL, inspect it with live web context when possible.\n'}\nDetermine whether the COMPLETE message is likely legitimate, suspicious, or a scam. Consider unsolicited crypto rewards or airdrops, unusually large/free token amounts, fake eligibility, claim deadlines/countdowns, pressure to act, requests to verify/connect a wallet, sign a transaction, approve a token, pay a fee, send crypto first, provide a seed phrase/private key, login credentials, or personal information. Also check impersonation, investment promises, fake endorsements, withdrawal claims, pig-butchering/drainer patterns, suspicious links/domains, and whether the claimed reward is plausible.\n\nIMPORTANT: Legitimate token names such as USDT, BTC, ETH, etc. do NOT make the surrounding message legitimate. The words airdrop, giveaway, or investment alone do NOT prove a scam either. Judge the full context and requested action.\n\nOnly report red flags actually present in the message or supported by live URL evidence. Do not invent sources or facts. Give a calibrated 0-100 risk score and explain the strongest evidence. CRITICAL SCORING RULE: A message that combines an unsolicited large/free crypto reward, a short deadline or countdown, a link to claim/verify, and a request to connect a wallet or approve/sign a transaction is HIGH RISK even when the named token is legitimate. For that combination, return risk_level=high, risk_score at least 90, is_likely_scam=true, and explain the combined social-engineering and wallet-drainer risk. For a benign message, explicitly say why it appears benign and keep the score low. Respond in plain English suitable for a non-technical user. Include source URLs only for sources actually consulted.`;
