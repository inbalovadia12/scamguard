...
const RISK_COLORS = { low: "hsl(var(--success))", medium: "hsl(var(--warning))", high: "hsl(var(--destructive))" };
...
export default function Analytics() {
  ...
  const exportCSV = () => {
    ...
    a.href = url;
    a.download = `vardin-analyses-${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
  ...
  analyses.forEach((a) => {
    ...
    (a.tactics_detected || []).forEach((t) => { tacticCounts[t] = (tacticCounts[t] || 0) + 1; });
  });
  ...
  const tacticData = Object.entries(tacticCounts).map(([tactic, count]) => ({ tactic, count })).sort((a, b) => b.count - a.count).slice(0, 6);
    ...
    return (
      <div className="max-w-2xl mx-auto space-y-6">
        <div className="text-center py-16 space-y-4">
          ...
          <Link to="/">
            <Button className="bg-gradient-to-r from-primary to-primary/80">Check a Message</Button>
          </Link>
        </div>
      </div>
    );
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <SummaryCard icon={BarChart3} label="Types Detected" value={Object.keys(typeCounts).length} color="text-success" />
      </div>
        ...
        <div className="p-5 rounded-2xl border border-border/50 bg-card">
          ...
          <div className="space-y-2">
            {highRisk.slice(0, 5).map((a) => (
              <div key={a.id} className="flex items-center gap-3 p-3 rounded-xl bg-destructive/5 border border-destructive/10">
                ...
                <span className="text-xs font-bold text-destructive flex-shrink-0">{a.risk_score}/100</span>
              </div>
            ))}
          </div>
        </div>
