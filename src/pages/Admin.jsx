...
function PhoneLookupBatchTab() {
  ...
  return <div className="space-y-4">
    ...
    </div>{(result.results || []).map((item, index) => <div key={item.number || index} className="bg-card rounded-2xl border border-border/50 p-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="font-medium break-all">{item.number}</p><p className="text-xs text-muted-foreground mt-1">{item.actual?.country || "Country unknown"}{item.actual?.business_name ? ` · ${item.actual.business_name}` : ""}</p></div>{item.ok ? <Badge className="bg-success/10 text-success border-success/20">Pass</Badge> : <Badge variant="destructive">Fail</Badge>}</div><div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mt-3 text-xs"><div><span className="text-muted-foreground">Status</span><div className="font-medium">{item.actual?.caller_id_status || "—"}</div></div><div><span className="text-muted-foreground">Risk score</span><div className="font-medium">{item.actual?.reputation_score ?? "—"}</div></div><div><span className="text-muted-foreground">Returned number</span><div className="font-medium break-all">{item.actual?.phone_number || "—"}</div></div><div><span className="text-muted-foreground">Expected</span><div className="font-medium">{item.failures?.length ? "See failure details" : "Matched"}</div></div><div><span className="text-muted-foreground">Time</span><div className="font-medium">{item.elapsed_ms != null ? item.elapsed_ms + " ms" : "—"}</div></div></div>{item.failures?.length > 0 && <div className="mt-3 text-xs text-destructive">{item.failures.map((failure, i) => <div key={i}>{failure}</div>)}</div>}{item.error && <p className="mt-3 text-xs text-destructive">{item.error}</p>}</div>)}</div>}
  </div>;
}
