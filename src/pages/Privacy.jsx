  ...
  return (
    <div className="w-full max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pb-10 sm:pb-16 break-words [overflow-wrap:anywhere]">
      ...
      <Section number={7} title="Call Guard, Audio, and Transcripts">
        ...
        <p>For uploaded recordings, the current application sends the recording through Base44-managed file handling and passes the resulting file URL to the Vardin <code>analyzeCallChunk</code> function. The backend sends the audio to AssemblyAI for transcription and speaker diarization. AssemblyAI is currently the <strong>only</strong> speech-to-text provider used by Call Guard. Audio submitted as raw bytes is first uploaded to AssemblyAI's temporary storage to obtain a URL it can fetch; audio already available at a public URL is passed to AssemblyAI directly without Vardin downloading it.</p>
        ...
        <p>Live audio is therefore not “local only,” “never transmitted,” or “never stored.” Audio is transmitted to Vardin's backend and, for transcription, to AssemblyAI. AssemblyAI processes audio to produce transcripts and speaker labels; its retention of submitted audio and transcript data is governed by its own privacy policy and account settings, which Vardin cannot control or verify from the application code. Vardin does not claim that production AssemblyAI retention settings have been verified unless separately confirmed. See the <a className="text-primary underline underline-offset-2" href="https://www.assemblyai.com/legal/privacy-policy" target="_blank" rel="noreferrer">AssemblyAI Privacy Policy</a> for its current data practices.</p>
      </Section>

      <Section number={8} title="Speaker Diarization and 'You' vs. 'Caller'">
        <p>Call Guard attempts to label transcript segments as <strong>you</strong>, <strong>caller</strong>, or <strong>unknown</strong>. The current implementation uses AssemblyAI's speaker diarization to separate speakers by voice, then maps the result based on the audio source: in system-audio/phone-call mode the captured audio is the caller's side, so all segments are labeled “caller”; in microphone and upload mode (mixed audio) speaker labels are estimated from speech-turn gaps, and the user can correct any label in the interface.</p>
      ...
      <Section number={16} title="How We Share Information">
        ...
        <BulletList items={[
          ...
          "AI and speech providers used for requested analysis, including Google Gemini, AssemblyAI and Groq.",
      ...
      <Section number={18} title="AI Providers and Training/Retention Controls">
        <p><strong>AssemblyAI:</strong> Call Guard sends audio to AssemblyAI for speech-to-text transcription and speaker diarization. AssemblyAI processes the audio and returns text and speaker-labeled segments. AssemblyAI's retention of audio and transcript data is governed by its privacy policy and account configuration, which Vardin cannot verify from the application code. Vardin does not claim that production AssemblyAI data-retention settings have been verified unless separately confirmed. <a className="text-primary underline underline-offset-2" href="https://www.assemblyai.com/legal/privacy-policy" target="_blank" rel="noreferrer">AssemblyAI Privacy Policy</a> · <a className="text-primary underline underline-offset-2" href="https://www.assemblyai.com/security" target="_blank" rel="noreferrer">AssemblyAI Security</a></p>
        <p><strong>Groq:</strong> Groq is used as a language model for Call Guard's fast in-call feedback (the <code>generateCallFeedback</code> function) and is <strong>not</strong> the speech-to-text provider. Groq's current data documentation says inference inputs and outputs are not retained by default, and that inputs/outputs may be temporarily retained for system reliability or abuse investigations for up to 30 days unless Zero Data Retention is enabled. Groq states that customer inputs and outputs are not used to train or fine-tune its models unless the customer explicitly grants permission or instructs it to. Groq also states that retained customer data is stored in Google Cloud Platform buckets in the United States. Vardin does not claim that production Groq ZDR is enabled unless that setting has been separately verified. <a className="text-primary underline underline-offset-2" href="https://console.groq.com/docs/your-data" target="_blank" rel="noreferrer">Groq data controls</a> · <a className="text-primary underline underline-offset-2" href="https://console.groq.com/docs/legal/services-agreement" target="_blank" rel="noreferrer">Groq Services Agreement</a></p>
