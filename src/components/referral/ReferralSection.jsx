    ...
    const load = async () => {
      try {
        const [data, codeResult] = await Promise.all([
          base44.entities.Referral.filter({ referrer_id: user.id }, "-created_date", 100),
          base44.functions.invoke("getReferralCode", {}),
        ]);
        setReferrals(data);
        if (codeResult?.data?.code) setReferralCode(codeResult.data.code);
      } catch {}
      setLoading(false);
    };
    load();
  ...
  return (
    <Card className="rounded-2xl border-border/50 p-6 space-y-5">
      ...
      <p className="text-sm text-muted-foreground">
        Share your unique invite link or referral code. When someone you bring activates a paid plan, you earn{" "}
      ...
      <div className="space-y-2">
        ...
        {referralCode && (
          <p className="text-xs text-muted-foreground">Your referral code: <span className="font-mono font-semibold text-foreground">{referralCode}</span></p>
        )}
      </div>
