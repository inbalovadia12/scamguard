    ...
    <div className="relative w-[min(92vw,470px)]">
      <motion.div animate={isMobile ? { rotate: 0 } : { rotate: [0, -1, 1, 0] }} transition={isMobile ? { duration: 0.2 } : { duration: 5, repeat: Infinity, ease: "easeInOut" }}>
        <Surface className="p-5 text-left">
          ...
          <div className="mt-4 grid grid-cols-2 gap-2">{tactics.map((t, i) => <motion.div key={t} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 + i * 0.1 }} className="rounded-lg border border-[#2f8f83]/15 bg-[#2f8f83]/[0.05] p-2.5 text-[8px] tracking-[0.08em] text-[#79c9bd]">{t}</motion.div>)}</div>
        </Surface>
      </motion.div>
    </div>
  );
...
function BrowserVisual() {
  return (
    <Surface className="w-[min(94vw,520px)] overflow-hidden text-left">
      <div className="flex items-center gap-1.5 border-b border-white/[0.08] bg-white/[0.025] px-3 py-2"><span className="h-1.5 w-1.5 rounded-full bg-white/20" /><span className="h-1.5 w-1.5 rounded-full bg-white/20" /><span className="h-1.5 w-1.5 rounded-full bg-white/20" /><div className="ml-3 flex-1 rounded-md bg-black/25 px-3 py-1.5 text-[8px] text-white/32">example-login-security.com</div><span className="text-[8px] text-[#2f8f83]">V</span></div>
      <div className="relative h-36 overflow-hidden bg-white/[0.018] p-5"><div className="h-3 w-28 rounded bg-white/10" /><div className="mt-4 h-2 w-52 rounded bg-white/[0.06]" /><div className="mt-2 h-2 w-40 rounded bg-white/[0.06]" /><motion.div initial={{ x: 70, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: 0.35, duration: 0.7, ease: EASE }} className="absolute right-4 top-5 w-40 rounded-xl border border-[#2f8f83]/25 bg-[#081311]/95 p-3 shadow-2xl"><div className="text-[8px] tracking-[0.16em] text-[#79c9bd]">VARDIN WARNING</div><div className="mt-2 text-[10px] text-white/75">Possible impersonation</div><div className="mt-1 text-[8px] text-white/35">Check before signing in.</div></motion.div></div>
    </Surface>
  );
}
    ...
    <div className="relative w-[min(88vw,340px)]">
      <motion.div animate={isMobile ? { y: 0, rotate: 0 } : { y: [0, -5, 0], rotate: [0, 0.5, 0] }} transition={isMobile ? { duration: 0.2 } : { duration: 4, repeat: Infinity, ease: "easeInOut" }}>
        <Surface className="overflow-hidden p-5">
          ...
          <motion.div initial={{ width: 0 }} animate={{ width: "100%" }} transition={{ delay: 0.5, duration: 0.8 }} className="mt-6 h-px bg-gradient-to-r from-transparent via-[#2f8f83]/60 to-transparent" />
          <div className="mt-4 flex items-center gap-2 text-[9px] text-white/48"><ShieldAlert className="h-3.5 w-3.5 text-[#79c9bd]" /> Risk context available</div>
        </Surface>
      </motion.div>
    </div>
  );
            ...
            >
              {scene.title.includes("<em>") ? (
                titleParts.map((part, partIndex) => part.startsWith("<em>")
                  ? <em key={partIndex} className="not-italic text-[#2f8f83]"><TypingTitle text={part.replace(/<\/?em>/g, "")} /></em>
