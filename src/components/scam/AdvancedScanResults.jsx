...
function QRDestinationCard({ data }) {
  ...
  const decoded = data.decoded_content || data.analysis?.decoded_content;
  const finalUrl = data.final_destination_url || data.analysis?.final_destination_url;
  const pageTitle = data.destination_title;
  const description = data.analysis?.destination_description;

  if (!decoded && !finalUrl) return null;

  return (
    <div className="rounded-2xl border-2 border-primary/30 bg-primary/5 p-5 space-y-3">
      <div className="flex items-center gap-2 text-primary">
        <QrCode className="w-5 h-5" />
        <h3 className="font-bold font-heading">QR Code Destination</h3>
      </div>
      {decoded && (
        <div className="space-y-1">
          <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Decoded Content</p>
          <p className="text-sm font-mono break-all bg-card rounded-lg px-3 py-2 border border-border/50">{decoded}</p>
        </div>
      )}
      {finalUrl && finalUrl !== decoded && (
        <div className="space-y-1">
          <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Final Destination (After Redirects)</p>
          <a href={finalUrl} target="_blank" rel="noopener noreferrer" className="flex items-start gap-1.5 text-sm font-mono break-all text-primary hover:underline bg-card rounded-lg px-3 py-2 border border-border/50">
      ...
      {pageTitle && (
        <div className="space-y-1">
          <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Page Title</p>
      ...
      {description && (
        <div className="space-y-1">
          <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Destination Description</p>
