...
import {
  Phone, Loader2, ShieldCheck, ShieldAlert, AlertTriangle, ListPlus,
...
export default function BulkPhoneScanner({ credits }) {
  ...
  const handleScan = async () => {
    ...
    const initialItems = numbers.map((n) => ({ number: n, status: "pending", result: null, error: null }));
    ...
    const workers = Array.from({ length: Math.min(MAX_CONCURRENCY, numbers.length) }, () => runWorker());
  ...
  const handleCancel = () => {
    ...
    setItems((prev) =>
      prev.map((it) =>
        it.status === "pending" || it.status === "scanning" ? { ...it, status: "cancelled" } : it
      )
    );
  };
  ...
  const handleExport = () => {
    ...
    const csv = ["Phone Number,Risk Score,Risk Level,Country,Carrier,Scam Reports,Summary"];
    done.forEach((it) => {
      const r = it.result;
      const summary = (r.summary || "").replace(/"/g, '""').replace(/\n/g, " ");
      csv.push(`"${it.number}",${r.reputation_score},${r.risk_level},"${r.country || ""}","${r.carrier || ""}",${r.scam_report_count || 0},"${summary}"`);
    });
  ...
  if (!credits.isPaid) {
    return (
      <PlanGate
        icon={Phone}
        title="Bulk Phone Scanner"
        description="Check multiple phone numbers at once. Each number is scanned against live web data for scam reports and reputation. Available on Plus and Premium plans."
        plan="Plus"
      />
    );
  }

  const detected = parseNumbers(input);
  const total = items.length;
  const done = completedCount;
  const progress = total > 0 ? (done / total) * 100 : 0;

  // Sort: done (by score desc) → scanning → pending → error → cancelled
  const order = { done: 0, scanning: 1, pending: 2, error: 3, cancelled: 4 };
  const sortedItems = [...items].sort((a, b) => {
    if (order[a.status] !== order[b.status]) return order[a.status] - order[b.status];
    if (a.status === "done" && b.status === "done") {
      return (b.result?.reputation_score || 0) - (a.result?.reputation_score || 0);
    }
    return 0;
  });
  ...
  return (
    <div className="space-y-5">
      <div className="text-center space-y-2 animate-slide-up">
        ...
        <h2 className="text-2xl sm:text-3xl font-bold tracking-tight font-heading">Bulk Phone Scanner</h2>
        <p className="text-muted-foreground text-base sm:text-lg max-w-md mx-auto hidden sm:block">
          Check multiple phone numbers at once. Results stream in as each number is scanned.
        </p>
      </div>
      ...
      <div className="bg-card rounded-3xl border border-border/50 shadow-sm p-4 sm:p-7 space-y-4 animate-slide-up anim-delay-2">
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <label className="text-sm font-medium">Paste phone numbers</label>
            <span className="text-xs text-muted-foreground">
              {detected.length}/{MAX_NUMBERS} detected
            </span>
          </div>
          <Textarea
            value={input}
            ...
            className="min-h-[120px] sm:min-h-[160px] text-base resize-none rounded-xl font-mono"
            disabled={scanning}
          />
          <p className="text-xs text-muted-foreground">
            Up to {MAX_NUMBERS} numbers at a time. Scans run {MAX_CONCURRENCY} at a time in the background — you can review results as they come in.
          </p>
        </div>
          ...
          {!scanning ? (
            <>
              ...
              >
                ...
                Scan {detected.length} Number{detected.length !== 1 ? "s" : ""}
              </Button>
              {items.length > 0 && (
              ...
              if (it.status === "pending" || it.status === "scanning" || it.status === "cancelled") {
                return (
                  <div key={originalIndex} className="flex items-center gap-3 p-3 rounded-xl border border-border/50 bg-muted/30">
                    ...
                    <span className="text-sm font-mono">{it.number}</span>
              ...
              if (it.status === "error") {
                return (
                  <div key={originalIndex} className="flex items-center gap-3 p-3 rounded-xl border border-destructive/30 bg-destructive/5">
                    ...
                    <span className="text-sm font-mono truncate">{it.number}</span>
                    <span className="text-xs text-destructive ml-auto truncate max-w-[180px]">{it.error}</span>
                  </div>
                );
              }
                ...
                <div key={originalIndex} className={`rounded-xl border ${risk.border} ${risk.bg} overflow-hidden`}>
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="text-sm font-mono font-medium truncate">{it.number}</p>
                      <p className="text-xs text-muted-foreground truncate">
                        {r.carrier || "Unknown carrier"}{r.country ? ` · ${r.country}` : ""}
                        {r.scam_report_count > 0 ? ` · ${r.scam_report_count} scam reports` : ""}
                      </p>
                    </div>
                      ...
                      {r.scam_categories?.length > 0 && (
                        <div className="flex flex-wrap gap-1.5">
                          {r.scam_categories.map((cat, i) => (
                            <span key={i} className="text-xs font-medium px-2 py-0.5 rounded-full bg-destructive/10 text-destructive">
                              {cat}
                            </span>
                          ))}
                        </div>
